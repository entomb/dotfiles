...
function set_svn_branch {
  ...
  #check for non commited changes
  if [[ ${svn_status} =~ $svn_any ]]; then
    BRANCH="${YELLOW}svn:(${branch}*)${COLOR_NONE} "
  else
    BRANCH="${LIGHT_GREEN}svn(${branch})${COLOR_NONE} "
  fi
...
function set_bash_prompt () {
  ...
  PS1="\n${BLUE}\u@${LIGHT_GRAY}\h${COLOR_NONE} [ \w ${BRANCH}\n${PROMPT_SYMBOL} "
  # Set the bash title on each commnad
